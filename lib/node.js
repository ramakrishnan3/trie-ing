 ...
 */
var Node = function (nodeObject) {
    /* The maximum score of all the values in this element
     * or its children */
    if (nodeObject) {
        this.score = nodeObject.score;
        this.values = nodeObject.values;
        this.sorted = nodeObject.sorted;
        this.leaf = nodeObject.leaf;
        this.children = {};
        if(!nodeObject.leaf && nodeObject.values.length) {
            nodeObject.values.forEach(child => this.children[child] = new Node(nodeObject.children[child]))
        }
    } else {
        this.score = 0;
...
Node.prototype.add = function (item, index, maxWidth) {
    ...
    // when the level goes beyond maxWidth, create a children array for each value of values having the node level as the indexing element
